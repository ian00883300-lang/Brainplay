...
const extreme=bank.extreme;delete bank.extreme;
...
html=html.replace(/^function vocabItems\(.*$/m,()=>`function vocabItems(diff,mode='all'){const source=diff==='extreme'?VOCAB_EXTREME:(VOCAB[diff]||VOCAB.starter);const rows=source.map(([word,zh,icon,pos,category,example,exampleZh,picture,usageKind])=>({word,zh,icon,pos,category,example,exampleZh,picture,usageKind:usageKind||'sentence'}));return mode==='picture'?rows.filter(x=>x.picture):rows}`);
...
html=html.slice(0,randomStart)+html.slice(randomStart,randomEnd).replaceAll('bank=vocabItems(diff)',"bank=vocabItems({easy:'starter',medium:'easy',hard:'medium',extreme:'hard'}[diff])").replace(/threeOptions=\(useZh=false\)=>\{[\s\S]*?\};if\(type==='vocabBlank'\)/,"threeOptions=(useZh=false)=>englishChoices(item,bank,3).map((x,i)=>({id:String.fromCharCode(65+i),item:x,label:useZh?x.zh:x.word}));if(type==='vocabBlank')")+html.slice(randomEnd);
...
const swPath=path.join(root,'sw.js');let sw=fs.readFileSync(swPath,'utf8').replace(/const CACHE_NAME = '[^']+';/,"const CACHE_NAME = 'brainplay-v4.60-phase63';");
...
console.log('Built Phase 63 HTML and offline cache revision.');
