...
const leveled=require('./expand65.cjs')(require('./twelve-levels.cjs')(groups,freq));
const report={baseCommit:'f746573fa80cfbf42d60a32f9e43b14431c5da5c',total:8000,counts:Object.fromEntries(Object.entries(leveled).map(([k,v])=>[k,v.length])),retained:7000-needed,added:needed+1000,phase65Added:1000,removed,source:'Original teaching content; ECDICT BNC frequency used only for ordering and editorial triage.',note:'Editorial learning stages, not certified CEFR levels. Same headword has one selected sense. Spelling alternatives are not used to fill the target.'};
...
console.log(JSON.stringify({counts:report.counts,retained:report.retained,added:report.added,removed:removed.length}));
